angular.module('school_erp')
    .factory('examServices', ['$http', 'globalServices', function ($http, globalServices) {
        ...
        examServices.setBulkMarks = function (dataValue, exam_sch_id, exam_paper_id, section_id, class_id) {
            //    console.log("values");
            //  console.log(dataValue);
            var test = {
                "studentsMarks": dataValue,
            };
            return $http({
                method: 'POST',
                url: globalServices.globalValue.baseURL + 'api/marksbulk_eval/' + exam_sch_id + "/" + exam_paper_id + '/' + section_id + '/' + class_id,
                data: test,
                headers: { 'Content-Type': 'application/json' },
            })
        };
